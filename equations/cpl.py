...
conditions = [CustomCondition(reparam_CPL(w_0, w_1))]
bundle_conditions = [CustomCondition(reparam_CPL())]
...
def H_CPL(z, w_0, w_1, Om_m_0, H_0, x):
    ...
    if callable(x):
        xs = x(z, w_0*shape, w_1*shape, to_numpy=True)
...
def int_eP(z, w_0=w_0, w_1=w_1):
    ...
    integral = lambda zz: exp(a) * ((zz + 1)**(b + 1)) * expint(b + 2, a / (zz + 1))
    return float(sympy.re(integral(z) - integral(0)))
...
fcnn_config = FCNNConfig(input_features=1, output_features=2, hidden_units=(32, 32), activation=torch.nn.Tanh, iterations=100_000, dimension_batch_size=64, lr=0.001)
...
bbb_config = BBBConfig(input_features=1, output_features=2, hidden_units=(32, 32), activation=torch.nn.Tanh, iterations=40_000, dimension_batch_size=100, prior_std=1, lr=0.001, likelihood_std=.01, res_likelihood_std=.01)
hmc_config = HMCConfig(input_features=1, output_features=2, hidden_units=(32, 32), activation=torch.nn.Tanh, samples=10_000, tune_samples=1000, chains=1, dimension_batch_size=100, target_acceptance_rate=0.95, prior_std=1, likelihood_std=.01, res_likelihood_std=.01)

fcnn_config_bundle = FCNNConfig(input_features=1, output_features=2, hidden_units=(32, 32), activation=torch.nn.Tanh, iterations=100_000, dimension_batch_size=(128, 2, 2), lr=0.001)
nlm_config_bundle = NLMConfig(prior_calibration_range=(.1, 2), prior_calibration_points=100, dimension_batch_size=(128, 16, 16), likelihood_std=.01, res_likelihood_std=.01)
bbb_config_bundle = BBBConfig(input_features=1, output_features=2, hidden_units=(32, 32), activation=torch.nn.Tanh, iterations=150_000, dimension_batch_size=(128, 2, 2), prior_std=1, lr=0.001, likelihood_std=.01, res_likelihood_std=.01, eb_dimension_batch_size=50)
hmc_config_bundle = HMCConfig(input_features=1, output_features=2, hidden_units=(32, 32), activation=torch.nn.Tanh, samples=10_000, tune_samples=1000, chains=1, dimension_batch_size=(128, 2, 2), target_acceptance_rate=0.95, prior_std=1, likelihood_std=.01, res_likelihood_std=.01, eb_dimension_batch_size=50, step_size=1e-7)
