...
def get_plot_data(eb=False, domain_type="test"):
    ...
    if domain_type == "test":
        ...
        x_test = np.linspace(cpl.coords_train_min, cpl.coords_train_max, 200).reshape(-1, 1)
    else:
        x_test = np.linspace(cpl.coords_train_max, cpl.coords_test_max, 200).reshape(-1, 1)
    ...
    print("Loading FCNN")
    solver = Solver1D.load("checkpoints/solver_cpl_fcnn.ndeq")
    solution = solver.get_solution()(x_test, to_numpy=True)
    fcnn_residuals = solver.get_residuals(x_test, to_numpy=True)

    print("Loading BBB")
    solver_bbb = torch.load(f"checkpoints/solver_cpl_bbb{eb}.pyro", map_location="cpu")
    ...
    print("Loading NLM")
    nlm_model = NLMModel.load(f"checkpoints/model_cpl_nlm{eb}.pt")
    ...
    print("Loading HMC")
    hmc_solver = torch.load(f"checkpoints/solver_cpl_hmc{eb}.pyro", pickle_module=dill, map_location="cpu")
    hmc_solver.diff_eqs = cpl.system
    hmc_solver.get_likelihood_std.device = "cpu"
    hmc_posterior_samples = torch.load(f"checkpoints/samples_cpl_hmc{eb}.pyro", pickle_module=dill, map_location="cpu")
...
def get_bundle_plot_data(eb=False, domain_type="test"):
    ...
    if domain_type == "test":
        ...
        t_test = np.linspace(cpl.coords_train_min, cpl.coords_train_max, cpl.bundle_plot_dimension_sizes[0])
    else:
        t_test = np.linspace(cpl.coords_train_max, cpl.coords_test_max, cpl.bundle_plot_dimension_sizes[0])
    ...
    solver = BundleSolver1D.load("checkpoints/solver_bundle_cpl_fcnn.ndeq")
    ...
    print("Loading BBB")
    solver_bbb = torch.load(f"checkpoints/solver_bundle_cpl_bbb{eb}.pyro", map_location="cpu")
    ...
    print("Loading NLM")
    nlm_model = NLMModel.load(f"checkpoints/model_bundle_cpl_nlm{eb}.pt")
    nlm_model.device = "cpu"
    nlm_model.get_likelihood_stds.device = "cpu"
    ...
    print("Loading HMC")
    hmc_solver = torch.load(f"checkpoints/solver_bundle_cpl_hmc{eb}.pyro", pickle_module=dill, map_location="cpu")
    hmc_solver.conditions = cpl.bundle_conditions
    hmc_solver.diff_eqs = cpl.system_bundle
    hmc_solver.get_likelihood_std.device = "cpu"
    hmc_posterior_samples = torch.load(f"checkpoints/samples_bundle_cpl_hmc{eb}.pyro", pickle_module=dill, map_location="cpu")
    n_hmc_samples = list(hmc_posterior_samples.values())[0].shape[0]
    batches = n_hmc_samples // batch_size
    hmc_samples = []
    hmc_residuals = []
...
@dill_dec_old("plot_data/cpl_best_fit.dill", "plot_data/cpl_best_fit_eb.dill")
def get_best_fit_plot_data(eb=False):
    eb = "_eb" if eb else ""
    w_0_fcnn, w_1_fcnn, Om_m_0_fcnn, H_0_fcnn = np.load("checkpoints/inverse_samples_bundle_cpl_fcnn_cc.npy").mean(axis=0)
    w_0_bbb, w_1_bbb, Om_m_0_bbb, H_0_bbb = np.load(f"checkpoints/inverse_samples_bundle_cpl_bbb{eb}_cc.npy").mean(axis=0)
    w_0_nlm, w_1_nlm, Om_m_0_nlm, H_0_nlm = np.load(f"checkpoints/inverse_samples_bundle_cpl_nlm{eb}_cc.npy").mean(axis=0)
    w_0_hmc, w_1_hmc, Om_m_0_hmc, H_0_hmc = np.load(f"checkpoints/inverse_samples_bundle_cpl_hmc{eb}_cc.npy").mean(axis=0)

    z = np.linspace(0, 2, 100).reshape(-1, 1)

    solver_fcnn = BundleSolver1D.load("checkpoints/solver_bundle_cpl_fcnn.ndeq")
    solver_fcnn.conditions = cpl.bundle_conditions
    solution_fcnn = solver_fcnn.get_solution()

    bbb_solver = torch.load(f"checkpoints/solver_bundle_cpl_bbb{eb}.pyro", map_location="cpu")
    ...
    hmc_solver.get_likelihood_std.device = "cpu"
    hmc_posterior_samples = torch.load(f"checkpoints/samples_bundle_cpl_hmc{eb}.pyro", pickle_module=dill)
...
def plot_cpl(eb=False, force=False):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 4), sharey=True, dpi=125)

    for i, method in enumerate(["BBB", "HMC"]):
    ...
    plt.savefig(f"figures/cpl/forward_cpl{eb_text}.png", bbox_inches='tight')
...
def plot_cpl_bundle_errors(method, eb=False, force=False):
    ...
    for w0 in np.linspace(0, 3, 4):
        ...
        fig.savefig(f"figures/cpl/bundle_error_cpl{eb_text}_{method}_test_{w0}.png", bbox_inches='tight')
        ...
        fig.savefig(f"figures/cpl/bundle_error_cpl{eb_text}_{method}_train_{w0}.png", bbox_inches='tight')
...
def plot_cpl_bundle_std_errors(method, eb=False, force=False):
    ...
    for w0 in np.linspace(0, 3, 4):
        ...
        fig.savefig(f"figures/cpl/bundle_std_error_cpl{eb_text}_{method}_test_{w0}.png", bbox_inches='tight')
        ...
        fig.savefig(f"figures/cpl/bundle_std_error_cpl{eb_text}_{method}_train_{w0}.png", bbox_inches='tight')
...
def plot_bundle_examples(eb=False):
    ...
    for w0 in map(int, np.linspace(0, cpl.bundle_plot_dimension_sizes[1]-1, 5)):
        ...
        for w1_idx in map(int, np.linspace(0, cpl.bundle_plot_dimension_sizes[2]-30, 5)):
            ...
            im3 = ax[1].plot(x[:, 0, 0], data["BBB"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], label="$w_1=" + f"{round(data['x'][2].reshape(cpl.bundle_plot_dimension_sizes)[0, w0, w1_idx], 2)}$")
            im4 = ax[2].plot(x[:, 0, 0], data["HMC"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], label="$w_1=" + f"{round(data['x'][2].reshape(cpl.bundle_plot_dimension_sizes)[0, w0, w1_idx], 2)}$")

            ax[1].fill_between(x[:, 0, 0], data["BBB"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]-2*data["BBB"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], data["BBB"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]+2*data["BBB"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], color=im1[0].get_color(), alpha=0.2, edgecolor=None)
            ax[2].fill_between(x[:, 0, 0], data["HMC"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]-2*data["HMC"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], data["HMC"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]+2*data["HMC"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], color=im1[0].get_color(), alpha=0.2, edgecolor=None)
        ...
        for i, m in enumerate(["FCNN", "BBB", "HMC"]):
            ax[i].set_xlabel("$z$", size=20)
        ...
        fig.suptitle("CPL Bundle Solutions"  + " for " + f"$w_0 = {round(data['x'][1].reshape(cpl.bundle_plot_dimension_sizes)[0, w0, 0], 2)}$" + (" (Error Bounds)" if eb else ""), size=26, y=1.05)
        plt.savefig(f"figures/cpl/bundle_examples_cpl{eb_text}_{w0}.png", bbox_inches='tight')

def plot_bundle_examples_all_params(eb=False):
    eb_text = "_eb" if eb else ""
    data = get_bundle_plot_data(eb=eb)
    x = data["x"][0].reshape(cpl.bundle_plot_dimension_sizes)
    ...
    for j, w0 in enumerate(map(int, np.linspace(0, cpl.bundle_plot_dimension_sizes[1]-1, 4))):
        for w1_idx in map(int, np.linspace(0, cpl.bundle_plot_dimension_sizes[2]-30, 5)):
            im1 = ax[j, 0].plot(x[:, 0, 0], data["FCNN"].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], label="$w_1=" + f"{round(data['x'][2].reshape(cpl.bundle_plot_dimension_sizes)[0, w0, w1_idx], 2)}$")
            im3 = ax[j, 1].plot(x[:, 0, 0], data["BBB"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], label="$w_1=" + f"{round(data['x'][2].reshape(cpl.bundle_plot_dimension_sizes)[0, w0, w1_idx], 2)}$")
            im4 = ax[j, 2].plot(x[:, 0, 0], data["HMC"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], label="$w_1=" + f"{round(data['x'][2].reshape(cpl.bundle_plot_dimension_sizes)[0, w0, w1_idx], 2)}$")

            ax[j, 1].fill_between(x[:, 0, 0], data["BBB"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]-2*data["BBB"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], data["BBB"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]+2*data["BBB"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], color=im1[0].get_color(), alpha=0.2, edgecolor=None)
            ax[j, 2].fill_between(x[:, 0, 0], data["HMC"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]-2*data["HMC"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], data["HMC"][0].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx]+2*data["HMC"][1].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], color=im1[0].get_color(), alpha=0.2, edgecolor=None)
            
            ax[j, 0].plot(x[:, 0, 0], data["analytic"].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], "--", color=im1[0].get_color())
            ax[j, 1].plot(x[:, 0, 0], data["analytic"].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], "--", color=im1[0].get_color())
            ax[j, 2].plot(x[:, 0, 0], data["analytic"].reshape(cpl.bundle_plot_dimension_sizes)[:, w0, w1_idx], "--", color=im1[0].get_color())

            ax[j, 1].set_ylim(ax[j, 0].get_ylim())
            ax[j, 2].set_ylim(ax[j, 0].get_ylim())

        w_0 = round(data['x'][1].reshape(cpl.bundle_plot_dimension_sizes)[0, w0, 0], 2)
        for i, m in enumerate(["FCNN", "BBB", "HMC"]):
            ax[-1, i].set_xlabel("$z$", size=26)
            ax[j, 0].set_ylabel("$x_{DE}$" + f" ($w_0=${w_0})", size=26)
            ax[0, i].set_title(m, size=26)
            ax[j, i].axvspan(x[:, 0, 0][0], x[:, 0, 0][int(cpl.bundle_plot_dimension_sizes[0]/2)], alpha=0.1, color='grey', label='Training Region')
            ax[j, i].set_xlim(z_min, z_max)
            
        #ax[j, 0].annotate(f"$w_0=${w_0}", xy=(0, 0.5), xytext=(-ax[j, 0].yaxis.labelpad - 5, 0), xycoords=ax[j, 0].yaxis.label, textcoords='offset points', size='large', ha='right', va='center')
        

    handles, labels = ax[-1, -1].get_legend_handles_labels()
    fig.legend(handles, labels, loc='lower center', ncol=6, bbox_to_anchor=(0.5, .03))
    fig.suptitle("CPL Bundle Solutions" + (" (Error Bounds)" if eb else ""), size=26, y=.93)
    plt.savefig(f"figures/cpl/bundle_examples_cpl{eb_text}.png", bbox_inches='tight')
...
def plot_hubble_forward(eb=False, force=False, ignore_nans=False):
    ...
    plt.savefig(f"figures/cpl/hubble_cpl{eb_text}{nan_text}.png", bbox_inches='tight')
...
def plot_hubble_best_fit(eb=False, force=False, ignore_nans=False):
    ...
    w_0_fcnn, w_1_fcnn, Om_m_0_fcnn, H_0_fcnn = np.load("checkpoints/inverse_samples_bundle_cpl_fcnn_cc.npy").mean(axis=0)
    w_0_bbb, w_1_bbb, Om_m_0_bbb, H_0_bbb = np.load("checkpoints/inverse_samples_bundle_cpl_bbb_cc.npy").mean(axis=0)
    w_0_nlm, w_1_nlm, Om_m_0_nlm, H_0_nlm = np.load("checkpoints/inverse_samples_bundle_cpl_nlm_cc.npy").mean(axis=0)
    w_0_hmc, w_1_hmc, Om_m_0_hmc, H_0_hmc = np.load("checkpoints/inverse_samples_bundle_cpl_hmc_cc.npy").mean(axis=0)
    ...
    plt.savefig(f"figures/cpl/hubble_cpl_best_fit{eb_text}{nan_text}.png", bbox_inches='tight')
...
def plot_hubble_bundle_errors(method, eb=False, force=False, ignore_nans=False):
    ...
    for w0 in np.linspace(0, 3, 4):
        ...
        cb3 = fig.colorbar(im3, ax=ax[2])
        cb3 = fig.colorbar(im4, ax=ax[3])
        fig.savefig(f"figures/cpl/hubble_bundle_error_cpl{eb_text}{nan_text}_{method}_test_{w0}.png", bbox_inches='tight')
        ...
        fig.colorbar(im7, ax=ax[2])
        fig.colorbar(im8, ax=ax[3])
        fig.savefig(f"figures/cpl/hubble_bundle_error_cpl{eb_text}{nan_text}_{method}_train_{w0}.png", bbox_inches='tight')
...
def plot_hubble_bundle_std_errors(method, eb=False, force=False, ignore_nans=False):
    ...
    for w0 in np.linspace(0, 3, 4):
        ...
        cb3 = fig.colorbar(im3, ax=ax[1])
        cb3 = fig.colorbar(im4, ax=ax[2])
        fig.savefig(f"figures/cpl/hubble_bundle_std_error_cpl{eb_text}{nan_text}_{method}_test_{w0}.png", bbox_inches='tight')
        ...
        fig.colorbar(im7, ax=ax[1])
        fig.colorbar(im8, ax=ax[2])
        fig.savefig(f"figures/cpl/hubble_bundle_std_error_cpl{eb_text}{nan_text}_{method}_train_{w0}.png", bbox_inches='tight')
...
def plot_calibration(data, name):
    ...
    plt.savefig(f"figures/cpl/{name}.png", bbox_inches='tight')
...
def plot_calibration_dts(data_train, data_test, data_ood, name):
    ...
    axes[0].annotate("BBB", size=26, xy=(0.5, 1), xytext=(0, 20), xycoords='axes fraction', textcoords='offset points', ha='center', va='baseline')
    axes[1].annotate("HMC", size=26, xy=(0.5, 1), xytext=(0, 20), xycoords='axes fraction', textcoords='offset points', ha='center', va='baseline')
    ...
    fig.legend(handles, labels, loc='lower center', ncol=5, bbox_to_anchor=(0.5, -0.16))

    fig.suptitle(f"CPL {method} Solution Calibration" + (" (Error Bounds)" if eb else ""), size=26, y=1.12)
    plt.savefig(f"figures/cpl/{name}.png", bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    #     plot_calibration(get_plot_data(domain_type=dt), f"calibration_cpl_forward_{dt}")
    #     plot_calibration(get_bundle_plot_data(domain_type=dt), f"calibration_cpl_bundle_{dt}")
    #     plot_calibration(get_plot_data(eb=True, domain_type=dt), f"calibration_cpl_forward_eb_{dt}")
    #     plot_calibration(get_bundle_plot_data(eb=True, domain_type=dt), f"calibration_cpl_bundle_eb_{dt}")

    plot_calibration_dts(*[get_plot_data(domain_type=dt) for dt in ["train", "test", "ood"]], "calibration_cpl_forward_all")
    plot_calibration_dts(*[get_plot_data(eb=True, domain_type=dt) for dt in ["train", "test", "ood"]], "calibration_cpl_forward_eb_all")
    plot_calibration_dts(*[get_bundle_plot_data(domain_type=dt) for dt in ["train", "test", "ood"]], "calibration_cpl_bundle_all")
    plot_calibration_dts(*[get_bundle_plot_data(eb=True, domain_type=dt) for dt in ["train", "test", "ood"]], "calibration_cpl_bundle_eb_all")

    # plot_bundle_examples_all_params()
    # plot_bundle_examples_all_params(True)
